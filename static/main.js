...
function checkMessages(){
  ...
  fetch(url+'/messages/getData').then(function(response) {
      ...
      for (let i in dataMessages['Messages']){
        document.getElementById('div'+Number(i)).innerHTML = ""
      }
      for (let i in dataMessages['Messages']){
        ...
        content.innerHTML = dataMessages['Messages'][i]['split_content'];
        ...
        console.log(dataMessages['Messages'][i]['filtre'])

        switch (Number(dataMessages['Messages'][i]['filtre'][0])) {
          case 0:
...
function sondage(){
  fetch(url+'/sondage').then(function(response) {
    ...
    if (percent <= 100){
      ...
      document.getElementById('non').innerHTML = 'non (' + Number(Math.round(100 - percent)) + '%)';
      document.getElementById('non').classList.remove('hidden');
    }
    else{
      document.getElementById('percent').style.width = '100%';
      document.getElementById('non').classList.add('hidden');
